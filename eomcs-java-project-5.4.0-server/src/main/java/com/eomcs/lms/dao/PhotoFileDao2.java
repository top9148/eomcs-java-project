...
public class PhotoFileDao2 {

  SqlSessionFactory sqlSessionFactory;

  public PhotoFileDao2(SqlSessionFactory sqlSessionFactory) {
  ...
  public int add(List<PhotoFile> photoFiles) throws Exception {
    SqlSession session = sqlSessionFactory.openSession();
    try {
      int count = session.insert("PhotoFileMapper.add", photoFiles);
      return count;
    } finally {
      session.close();
    }
